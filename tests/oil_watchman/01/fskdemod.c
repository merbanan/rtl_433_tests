...
void transition(int level, int samples, int halfbits)
{
	...
	if (framepos == -2) {
		if (halfbits == 3) {
			if (debug)
				printf("Frame started (%s). 3 high HB ends at %f\n",  level? "normal":"inverted", (float)samples/SFREQ);
			invertframe = !level;
			framestart = samples;
			framepos = -1;
...
int main(int argc, char **argv)
{
	...
	while (read(infd, b, 2) == 2) {
		s++;
		this = (int16_t)(b[0] + (b[1] << 8));
//		printf("%d: (%f): %d\n", s, (float)s/SFREQ, this);
		if (this > 0) {
			hi_bits++;
			if (lo_bits >= 180) {
				transition(0, s, (lo_bits + 100) / 250);
				laststable = s;
			} else if (lo_bits && framepos != -2 && s - laststable > 50) {
				if (debug)
			...
			lo_bits++;
			if (hi_bits >= 180) {
				transition(1, s, (hi_bits + 100) / 250);
				laststable = s;
			} else if (hi_bits && framepos != -2 && s - laststable > 50) {
				if (debug)
					printf("Unstable reset at %f\n", (float)s/SFREQ);
				framepos = -2;
			}
