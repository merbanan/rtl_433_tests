...
leak detector (see `../govee_h5059`) and H5310 pool/spa thermometer (see
`../govee_h5310`).

See https://github.com/merbanan/rtl_433/pull/3568 (decoder by Paul Antaki)
and https://github.com/merbanan/rtl_433_tests/pull/505 (these captures, same
author).
...
- Modulation: FSK_PULSE_PCM, short/long width 100 us, reset_limit 2000 us.

## Frame types

All 3 captures are in `01/`.
...
All 3 captures verified against the actual decoder before being added,
including cross-checking for false-positive cross-talk against the existing
govee_h5059 and govee_h5310 regression captures (none observed via the real
`-r` capture pipeline).
