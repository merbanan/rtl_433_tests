...
| `water_timer_0x0a` | `0x0a` valve -> gateway telemetry | `Bresser-WaterTimer` | the same moisture/temperature block **relayed** by the valve (47 %, 66.0 F) — the valve has no soil probe. `device_type`/`sensor_number` are ids (both 6 here), and `soil_rssi` = 23 is the raw register the valve measured for the soil sensor, the figure the app shows as that sensor's signal. |
...
| `status_report_0x02_0x82` | Water Timer `0x02` status report **+** gateway `0x82` reply (same counter) | `Bresser-WaterTimer`, `Bresser-Gateway` | A report/reply pair in one capture: the Water Timer pushes its own run state (`0x02`, len 15) and the gateway answers with `0x82` = request type\|0x80, echoing `msg_counter` 6 — the reply-flag convention that also governs the acks. `gateway_time` = 15269896 is a monotonic u24 the base keeps cloud-synced (pulling the base's batteries on 2026-07-20 reset it to 0; it re-synced from the cloud on boot, not free-running uptime; ~1.07/s). Its exact unit is unconfirmed, so it is emitted raw and labelled `Status response`, not `time`. |
...
| `config_channel_0x20` | `0x20` config change, 3-byte form | `Bresser-Gateway` | The longer `0x20` that also reports which setting changed: field id `0x04` = RF Communication Channel, new value 3, emitted as `rf_channel`. Recorded at 433.2 MHz, the Channel 3 uplink. |
| `run_start_0x21_0xa1` | gateway `0x21` run start **+** valve `0xa1` reply | `Bresser-Gateway`, `Bresser-WaterTimer` | A manual **misting** run started from the app: the command carries `mode=2` (misting) and `duration_s=120`, matching the 2 min set in the app, and the valve answers with `trigger=0x22` (manual+misting), `duration_s=120`, `remaining_s=121` — the same `duration+1` start value as the `0x02` countdown. The duration is little-endian 16-bit (`78 00`). |
...
| `soil_direct_0x03` | `0x03` soil sensor -> gateway, direct | `Bresser-SoilMoisture` | The sensor talking straight to the gateway with "Relay Communication" turned off, so no valve relay is involved and the frame carries no sensor address. Same battery/moisture/temperature block as `0x09` in its compact form. Captured with an SDR beside the sensor — direct mode also moves its transmit frequency, which is why it is easy to miss. |
| `valve_ack_0x89` | valve `0x89` ack of the soil `0x09` | `Bresser-WaterTimer` | The other half of the soil relay: the Water Timer acknowledging the sensor's telemetry, `target_id` being the soil sensor's own id. Recorded at 434.6 MHz — everything addressed *to* the soil sensor uses that fixed receive channel, not the 433 MHz links. |
