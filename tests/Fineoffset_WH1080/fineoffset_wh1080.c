 ...
 * which seems to be kind of a rebranded Fine Offset WH1080 Weather Station. 
 *
 ...
 * The module seems to work fine atm, decoding all of these data as read from the original console (there is some minimal difference
 * sometime on the decimals due to the different architecture of the console processor, which is less precise).
 * The pressure (barometer) sensor is enclosed in the indoor console unit, NOT in the outdoor wireless sensors group. 
 * That's why it's NOT possible to get pressure data by wireless communication. If you need pressure data you could try 
 * an Arduino/Raspberry solution wired with a BMP180 or BMP085 sensor.
 *
 * Data are trasmitted in a 48 seconds cycle (data, then wait 48 seconds, then data...).
 * 
 * The module is also capable to decode the DCF77 time signal sent by the wireless time signal decoder: 
 * around the minute 59 of every even hour the sensor's TX stops sending weather data, 
 * and after some silence it starts to send just time data for some minute. Then it starts again with meteo data as usual.
 * To recognize message type (weather or time) you can use the 'msg_type' field on json:
 * msg_type 0 = weather data
 * msg_type 1 = time data
 *
 * Living in Europe I can only test DCF77 time decoding, so if you live outside Europe and you find garbage instead of correct time,
 * you should disable time decoding (or better, try to implement a more complete decoding system :) ).
 *
 ...
 * I've tested the module with a Froggit WH1080 on 868.3 Mhz, using '-f 868140000' as frequency parameter and it works fine, 
 * I don't know if it works also with other versions -and, generally speaking, with ALL of the rebranded versions of this product-. 
 * I guess it *should* work... Just give it a try! :)
 *
 * 
 * ***TODO***: check if negative temperatures values are OK (no real winter this year where I live, so cannot test...) .
 * 
 *
...
//-------- GETTING WEATHER SENSORS DATA -------------------------------------------------
    ...
    const char device_id = get_device_id(br);
...
if (msg_type == 0) {
    ...
    data = data_make("time",          "",               DATA_STRING, time_str,
                     ...
                     "id",            "StationID",      DATA_FORMAT, "%02d",	DATA_INT,    device_id,
                     ...
                     "speed",         "Wind speed",     DATA_FORMAT, "%.01f",	DATA_DOUBLE, speed,
                     "gust",          "Wind gust",      DATA_FORMAT, "%.01f",	DATA_DOUBLE, gust,
